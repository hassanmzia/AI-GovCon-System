...
async def generate_diagrams(state: SolutionArchitectState) -> dict:
    ...
    content = await _llm(
        system=(
            "You generate Mermaid.js diagrams. Follow the EXACT syntax below.\n\n"
            "RULES:\n"
            "1. First line: graph TD\n"
            "2. Node IDs: ONLY letters, numbers, underscores. No spaces, no slashes, no dots, no hyphens, no parentheses.\n"
            "   WRONG: CI/CD Pipeline, Application Execution (Lambda), real-time\n"
            "   RIGHT: CICD_Pipeline, AppExecution, RealTime\n"
            "3. ALWAYS give nodes a label: NodeId[Human Readable Label]\n"
            "4. Arrows: A --> B or A -->|label| B\n"
            "5. Grouping: subgraph Title then nodes then end\n"
            "6. No nested subgraphs. No subgraph inside brackets.\n"
            "7. No semicolons. No C4 syntax. No actor() or container().\n"
            "8. Each diagram MUST have 6-10 nodes with labeled arrows between them.\n"
            "9. Do NOT use pipe | character inside node labels []. Only use | in arrow labels -->|like this|.\n"
            "10. Wrap each diagram in ```mermaid code blocks.\n\n"
            "EXAMPLE — every diagram must look like this:\n"
            "```mermaid\n"
            "graph TD\n"
            "    User[End Users] -->|HTTPS| LB[Load Balancer]\n"
            "    LB --> WebApp[Web Application]\n"
            "    WebApp -->|Queries| DB[(PostgreSQL Database)]\n"
            "    WebApp -->|Publishes| MQ[Message Queue]\n"
            "    MQ -->|Consumes| Worker[Background Worker]\n"
            "    Worker -->|Writes| DB\n"
            "    WebApp -->|Calls| ExtAPI[External API Gateway]\n"
        ...
        human=(
            ...
            f"Selected Frameworks: {', '.join(state['selected_frameworks'])}\n\n"
            "Generate exactly 5 diagrams. IMPORTANT: Each diagram MUST have at least 6 nodes "
            "connected by labeled arrows. Use CamelCase node IDs with labels in brackets.\n\n"
            "### Diagram 1: System Context\n"
            "Show: users, admin, the main system, government agency, database, external API, auth service, monitoring. "
            "At least 7 nodes with labeled arrows.\n\n"
            "### Diagram 2: Container Architecture\n"
            "Show: web frontend, API gateway, app server, database, cache, message queue, worker service, file storage. "
            "Use subgraph to group. At least 8 nodes.\n\n"
            "### Diagram 3: Data Flow\n"
            "Show: data ingestion, validation, transformation, storage, processing, analytics, reporting, API output. "
            "At least 7 nodes.\n\n"
            "### Diagram 4: Security Architecture\n"
            "Show: WAF/firewall, load balancer, IAM/auth, encryption layer, SIEM, audit logging, key management, network zones. "
            "At least 7 nodes.\n\n"
