...
async def generate_diagrams(state: SolutionArchitectState) -> dict:
    ...
    content = await _llm(
        system=(
            "You generate Mermaid.js diagrams. Every diagram MUST use `graph TD` on the first line.\n\n"
            "STRICT RULES:\n"
            "1. First line of every diagram must be exactly: graph TD\n"
            "2. Node IDs must be simple letters/numbers/underscores ONLY. NO slashes, dots, or hyphens in IDs.\n"
            "   WRONG: CI/CDPipeline, my.node, real-time\n"
            "   RIGHT: CICDPipeline, MyNode, RealTime\n"
            "3. Node labels go in square brackets: NodeId[Label Text]\n"
            "4. Arrows: A --> B or A -->|label text| B\n"
            "5. Grouping: subgraph Title\\n    nodes here\\n    end\n"
            "6. Do NOT nest subgraphs inside other subgraphs.\n"
            "7. Do NOT put subgraph inside node brackets like A[subgraph ...]\n"
            "8. Do NOT use semicolons.\n"
            "9. Do NOT use C4Context, actor(), container(), component(), boundary().\n"
            "10. Do NOT use commas to list multiple targets: WRONG: A --> B, C  RIGHT: A --> B and A --> C on separate lines.\n"
            "11. Keep diagrams simple: 5-12 nodes maximum per diagram.\n"
            "12. Wrap each diagram in ```mermaid ... ``` fenced code blocks.\n\n"
            "EXAMPLE (follow this pattern exactly):\n"
            "```mermaid\n"
            "graph TD\n"
            "    User[End Users] -->|Uses| System[Main System]\n"
            "    Admin[Administrators] -->|Manages| System\n"
            "    System -->|Reads| DB[(Database)]\n"
        ...
        human=(
            ...
            f"Selected Frameworks: {', '.join(state['selected_frameworks'])}\n\n"
            "Generate exactly 5 diagrams. Each diagram MUST start with `graph TD`. "
            "Keep each diagram simple (5-10 nodes). Use only alphanumeric node IDs.\n\n"
            "### Diagram 1: System Context\n"
            "Show users, the main system, and external systems. 5-8 nodes max.\n\n"
            "### Diagram 2: Container Architecture\n"
            "Show services, databases, and APIs. Use subgraph for grouping. 6-10 nodes max.\n\n"
            "### Diagram 3: Data Flow\n"
            "Show data flow from input to output. 5-8 nodes max.\n\n"
            "### Diagram 4: Security Architecture\n"
            "Show security layers: firewall, IAM, encryption. 5-8 nodes max.\n\n"
            "### Diagram 5: Deployment Architecture\n"
            "Show cloud infrastructure and deployment. 5-8 nodes max.\n\n"
