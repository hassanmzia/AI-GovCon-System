...
async def generate_diagrams(state: SolutionArchitectState) -> dict:
    ...
    content = await _llm(
        system=(
            ...
            "directly by the standard mermaid library. "
            "Wrap each diagram in a fenced code block with 'mermaid' as the language.\n\n"
            "CRITICAL MERMAID SYNTAX RULES — FOLLOW EXACTLY:\n"
            "- ONLY use standard Mermaid diagram types: graph TD, graph LR, flowchart TD, flowchart LR, sequenceDiagram, stateDiagram-v2\n"
            "- Do NOT use C4Context, C4Container, C4Deployment, actor(), container(), component(), boundary(), container_system() — these are NOT supported\n"
            "- Do NOT use semicolons at the end of lines\n"
            "- In `class` statements, NO spaces after commas: `class A,B,C styleName` (correct)\n"
            "- Use `graph TD` or `flowchart TD` for architecture diagrams with `subgraph` for grouping\n"
            "- Use simple alphanumeric node IDs (no dots, slashes, or special chars in IDs)\n"
            "- Node labels go in square brackets: `NodeId[Label Text]`\n"
            "- Use `-->` for arrows, `-->|label|` for labeled arrows\n"
            "- Use `subgraph Title ... end` for grouping related components\n"
            "- Use `style NodeId fill:#color` or `classDef` for styling\n\n"
            "EXAMPLE of a correct system context diagram:\n"
            "```mermaid\n"
            "graph TD\n"
            "    User[End Users] -->|Uses| System[Main System]\n"
            "    Admin[Administrators] -->|Manages| System\n"
        ...
        human=(
            ...
            f"Selected Frameworks: {', '.join(state['selected_frameworks'])}\n\n"
            "Generate the following 5 diagrams using ONLY `graph TD` or `flowchart TD` syntax "
            "(NO C4Context/C4Container/actor/component syntax):\n\n"
            "### Diagram 1: System Context\n"
            "Show external actors, the main system, and external dependencies using graph TD with subgraphs.\n\n"
            "### Diagram 2: Container Architecture\n"
            "Show major services, databases, APIs, and their interactions using graph TD with subgraphs for logical grouping.\n\n"
            "### Diagram 3: Data Flow\n"
            "Show how data flows through the system from ingestion to consumption.\n\n"
            "### Diagram 4: Security Architecture\n"
            "Show security layers, IAM, encryption, and compliance boundaries using nested subgraphs.\n\n"
            "### Diagram 5: Deployment Architecture\n"
            "Show cloud infrastructure, availability zones, and network topology.\n\n"
            "For each diagram provide:\n"
            "1. Diagram title\n"
            "2. The complete Mermaid.js code in a ```mermaid ... ``` block (MUST start with `graph TD` or `flowchart TD`)\n"
